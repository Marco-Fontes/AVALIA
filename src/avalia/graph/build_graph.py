...
from avalia.graph.budget import RunRegistry
...
def build_avalia_graph(
    *,
    ...
    # T3.2 + T-805 (v1.4): recursos POR EXECUÇÃO (chave = thread_id) — o medidor de orçamento,
    # que aplica os tetos antes de cada chamada de juiz, e o cache de juízo, compartilhado entre
    # as 7 dimensões e a reconciliação. Recriados na ingestão; nada vaza entre avaliações mesmo
    # que o grafo compilado seja reusado (ex.: serviço). Cache só quando há juiz (gateway).
    registry = RunRegistry(with_cache=gateway is not None)

    g: Any = StateGraph(AvaliaState)
    g.add_node("ingest", make_ingest_node(registry))
    g.add_node("index", n1_index)
    g.add_node("classify", n2_classify)
    g.add_node("select_weights", n3_select_weights)
    g.add_node("detect_divergence", make_detect_divergence_node(gateway, registry))
    ...
    g.add_node("build_report", make_build_report_node(repository, registry))

    dim_nodes = [f"dim_{d.value}" for d in Dimension]
    for d, name in zip(Dimension, dim_nodes, strict=True):
        g.add_node(name, make_dimension_node(d, gateway, registry))
